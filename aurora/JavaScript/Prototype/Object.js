...
Object.is = function (_ob_) {
  try {
    return _ob_ instanceof Object || typeof _ob_ == 'object' ? _ob_ : String.is(_ob_) ? JSON.parse(_ob_) : null;
  } catch (e) {
    return null;
  }
};
...
Object.merge = function _(...o) {
  return o.reduce((_, _o) => {
    return !_o || typeof _o !== 'object' ? _ : Object.entries(_o).reduce((target, info) => {
      if (info[1] && typeof info[1] === 'object' && typeof target[info[0]] === 'object') target[info[0]] = Object.merge(target[info[0]], info[1]);else target[info[0]] = info[1];
      return target;
    }, _);
  }, {});
};
  ...
  return objects.filter(Object.is).reduce(function (target, current) {
    Object.keys(current).forEach(function (key) {
      if (deletes.indexOf(key) < 0) {
        if (!(key in target)) target[key] = current[key];else {
