...
export default function BrowserDB(props = {}, success = null) {
  ...
  return new Promise((Resolved, Failed) => {
    ...
    Server.onupgradeneeded = event => {
      ...
      if (!_database.objectStoreNames.contains('BrowserDB@buckets')) _database.createObjectStore('BrowserDB@buckets', {
        keyPath: 'name',
        unique: true
      });
  ...
  }).then(connect => !options.persistence ? connect.close() : connect).catch(([message, event]) => {
    switch (message) {
      case 'connection_blocked':
        if (options.onBlocked) options.onBlocked(event);else options.error(message, event);
        break;
