class AwaitPromises {
  constructor(executor, reject) {
    ...
    try {
      executor.call(this, d => this.resolve(d), d => this.reject(d));
    } catch (error) {
      (reject || (err => {
        throw err;
      })).call(this, error);
  ...
  watch(name, handler, ...arg) {
    this.watchers.push({
      ...
      arg,
      target: this,
      ...(typeof name == 'object' ? name : typeof name == 'function' ? {
        handler: name,
        name: 'then'
      } : {})
    });
  ...
  then(callback) {
    return this.watch('then', callback);
  }

  catch(callback) {
    return this.watch('catch', callback);
  }
    ...
    while (i < this.watchers.length) {
      ...
      switch (this.status) {
        case 'resolved':
          if (info.name !== 'catch') {
            try {
              this.value = info.handler.call(info.target, ...(info.arg.length ? info.arg : [this.value]));
              delete this.error;
            } catch (err) {
              this.error = err;
              this.status = 'rejected';
