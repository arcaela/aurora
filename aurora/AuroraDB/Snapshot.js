...
function mergeDeep(target, ...sources) {
  ...
  const source = sources.shift(),
        clone = is_obj(target) ? { ...target
  } : null;

  if (clone && is_obj(source)) {
    for (const key in source) {
      clone[key] = is_obj(clone[key]) && is_obj(source[key]) ? mergeDeep(clone[key], source[key]) : Array.isArray(clone[key]) && Array.isArray(source[key]) ? clone[key].concat(...source[key]) : source[key];
    }
  }
...
export default class Snapshot {
  ...
  data(...o) {
    const _object = o.length ? o[0] : this.props,
          result = !is_obj(_object) ? _object : Object.entries(_object).filter(([k, v]) => v !== null).reduce((_, [key, value]) => {
  ...
  get(key, opt = null) {
    return get(this.props, path(key), opt);
  }
  ...
  update(key, value) {
    ...
    if (is_obj(prev) && is_obj(value)) value = mergeDeep(prev, value);
  ...
  bucket(name, data = null) {
    ...
    if (data !== null && !is_obj(data)) throw new Error('Intentas crear un almacen con datos noObject, usa bucketForce');
